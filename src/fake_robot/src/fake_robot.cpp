...
private:
  ...
  void publish_observed_landmarks() {
    ...
    pc->header.stamp = now;
    //pc->header.frame_id = "map";
    pc->header.frame_id = "robot_noisy";
    pc->height = 1;
    ...
    for (size_t i = 0; i < observed.size(); ++i) {
      ...
      // 1. Calculate relative distance from GT Robot
      double dx_global = observed[i].first.x - robot_x_; // GT Position
      double dy_global = observed[i].first.y - robot_y_;
      
      // 2. Rotate to Robot Frame (using GT Theta)
      double c = std::cos(robot_theta_gt_); // GT Orientation
      double s = std::sin(robot_theta_gt_);
      
      double rel_x = dx_global * c + dy_global * s;
      double rel_y = -dx_global * s + dy_global * c;

      // 3. Publish RELATIVE coordinate
      float* ptr = (float*)&pc->data[i * pc->point_step];
      ptr[0] = rel_x + noise_x; 
      ptr[1] = rel_y + noise_y;
